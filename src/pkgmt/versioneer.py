...
class VersionerNonSetup:
    ...
    def __init__(self, version_package, project_root='.'):
        ...
        for path, dirs, files in os.walk(project_root):
            if version_package in dirs:
                self.PACKAGE = Path(path, version_package)

        self.package_name = version_package
    ...
    def current_version(self):
        ...
        with open(self.PACKAGE / '_version.py', 'rb') as f:
            VERSION = str(
                ast.literal_eval(
                    _version_re.search(f.read().decode('utf-8')).group(1)))
    ...
    def commit_version(self, new_version, msg_template, tag=False):
        ...
        # replace new version in __init__.py
        replace_in_file(self.PACKAGE / '_version.py', current, new_version)
